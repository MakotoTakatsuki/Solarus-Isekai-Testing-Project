...
local inventory_2_submenu = submenu:new()
...
function inventory_2_submenu:on_started()
  ...
  local index = self.game:get_value("pause_inventory_2_last_item_index") or 0
  local row = math.floor(index / 7)
  local column = index % 7
  self:set_cursor_position(row, column)
end

function inventory_2_submenu:on_finished()
...
function inventory_2_submenu:set_cursor_position(row, column)

  self.cursor_row = row
  self.cursor_column = column

  local index = row * 7 + column
  self.game:set_value("pause_inventory_2_last_item_index", index)
...
function inventory_2_submenu:get_selected_index()

  return self.cursor_row * 7 + self.cursor_column
end

function inventory_2_submenu:is_item_selected()

  local item_name = item_names[self:get_selected_index() + 1]
  return self.game:get_item(item_name):get_variant() > 0
end

function inventory_2_submenu:on_command_pressed(command)
...
function inventory_2_submenu:on_draw(dst_surface)
...
function inventory_2_submenu:show_info_message()
...
function inventory_2_submenu:assign_item(slot)
...
function inventory_2_submenu:is_assigning_item()
...
-- is being closed.
function inventory_2_submenu:finish_assigning_item()
